...
import { SortFilterControl } from "@/components/file-manager/SortFilterControl";
...
export default function DashboardPage() {
  ...
  const handleRename = async (fileId, newName) => {
    try {
      ...
      const item =
        [...files, ...folders].find((i) => i._id === fileId) ||
        (isSearching ? searchResults.find((i) => i._id === fileId) : null);
  ...
  // Navigate to any folder (handling Search jumps)
  const navigateToFolder = async (folderId) => {
    ...
    // Check if folder is in current view (child of current)
    const localFolder = folders.find((f) => f._id === folderId);
    if (localFolder) {
      // Navigate down
      setCurrentFolderId(folderId);
      setBreadcrumbPath((prev) => [
        ...prev,
        { id: folderId, name: localFolder.name },
      ]);
    } else {
      // Navigate jump (fetch details)
      try {
        // Determine name - we need to fetch it
        const folder = await getFolder(folderId);
        setCurrentFolderId(folderId);
        // For a jump, we reset breadcrumbs to just this folder (since we don't know the path)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header
        ...
        onFileSelect={(file) => {
          navigateToFolder(file.parentId || "root");
        }}
      />
          ...
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            {loading ? (
              ...
              <FileList
                files={
                  isSearching
                    ? searchResults.filter((f) => !f.isFolder)
                    : sortedFiles
                }
                folders={
                  isSearching
